...
var TrackballControls = function (object, domElement) {
	...
	// methods
	this.handleResize = function (full) {
		if (this.domElement === document) {
			this.screen.left = 0;
			this.screen.top = 0;
			this.screen.width = document.getElementById("canvas3d").innerWidth;
			this.screen.height = document.getElementById("canvas3d").innerHeight;
		} else {
			var box = this.domElement.getBoundingClientRect();
			// adjustments come from similar code in the jquery offset() function
			var d = this.domElement.ownerDocument.documentElement;
			this.screen.right = box.left + window.pageXOffset - d.clientLeft;
	...
	this.panCamera = (function () {
		...
		return function panCamera() {
			...
			var panDeltaY = _panEnd.y - _panStart.y;
			var panDeltaX = _panEnd.x - _panStart.x;

			/**if(_this.target.x < -2 && panDeltaX > 0){
				return;
			}else if(_this.target.x > 2 && panDeltaX < 0){
				return;
			}else if(_this.target.y > 2 && panDeltaY > 0){
				return;
			}else if(_this.target.y < -2 && panDeltaY < 0){
				return;
			} */
	...
	function keydown(event) {
		...
		if (_state !== STATE.NONE) {
			...
			_state = STATE.ZOOM;
		} else if (event.keyCode === _this.keys[STATE.PAN] && !_this.noRotate) {
			_state = STATE.ROTATE;
		}
	}
	...
	function mousedown(event) {
		...
		if (_state === STATE.ROTATE && !_this.noPan) {
			_panStart.copy(getMouseOnScreen(event.pageX, event.pageY));
			_panEnd.copy(_panStart);
		} else if (_state === STATE.ZOOM && !_this.noZoom) {
			_zoomStart.copy(getMouseOnScreen(event.pageX, event.pageY));
			_zoomEnd.copy(_zoomStart);
		} else if (_state === STATE.PAN && !_this.noRotate) {
